          ...
          [
            {
              ...
              content:
                "导航栏中的层级定义,与原型所在文件系统的目录结构一一对应。",
            },
            {
              specId: "navigateName",
              content:
                "导航栏中的原型名称定义，来源于原型页面<html>标签的 data-nav-name 数据属性值",
            },
            {
              specId: "navigateSeq",
              content:
                "导航栏位置定义，来源于原型页面<html>标签的 data-nav-seq 数据属性值",
            },
            {
              specId: "pageNote",
              content:
...
export function registerGetSpecTool(server: McpServer) {
  server.registerTool(
    "getSpec",
    {
      ...
      description:
        "检索 MCP-Prototype 使用规范，了解如何在原型里增加**标记**和**页面辅助说明**",
      inputSchema: {},
    },
    async () => {
