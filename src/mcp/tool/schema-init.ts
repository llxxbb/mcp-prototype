...
export const InitArgsSchema = z.object({
	projectPath: z.string().optional().describe('当前项目的路径, 第一次调用时必须指定'),
	prototypeRoot: z
		.string()
		.optional()
		.default('')
	...
	marker: z
		.object({
			position: z
				...
				.optional()
				.default(0)
				.describe('标记位置：0(左上),1(中上),2(右上),3(左下),4(中下),5(右下),6(左),7(右)'),
			disable: z.boolean().optional().default(false).describe('是否禁用所有标记')
		})
		.optional()
		.default({})
		.describe('标记相关设置')
